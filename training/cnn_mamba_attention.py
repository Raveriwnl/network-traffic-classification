...
class MambaLayer(nn.Module):
    """Single-direction Mamba processing for causal sequence modeling."""

    def __init__(self, d_model, d_state=16, d_conv=3, expand=2, dropout=0.0):
        super().__init__()
        self.mamba = MambaS6Layer(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand)
...
class MambaBlock(nn.Module):
    def __init__(self, d_model=64, d_state=16, dropout=0.15, drop_path=0.0):
        ...
        self.ssm = MambaLayer(d_model=d_model, d_state=d_state, dropout=dropout)
...
class CNNMambaAttention(nn.Module):
    """CNN + single-direction Mamba classifier for inputs of shape (B, 1000, 10)."""
...
def build_cnn_mamba_attention_model(
    input_shape=(1000, 10),
    num_classes=8,
    num_mamba_layers=2,
    mamba_dropout=0.15,
