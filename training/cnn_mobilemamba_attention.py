...
class MobileMambaLayer(nn.Module):
    """Single-direction MobileMamba processing for sequence modeling."""

    def __init__(self, d_model, d_state=16, d_conv=3, expand=2, dropout=0.0):
        super().__init__()
        self.mobile_mamba = MobileMambaSSMLayer(
            d_model=d_model,
            d_state=d_state,
            d_conv=d_conv,
            expand=expand,
    ...
    def forward(self, x):
        return self.dropout(self.mobile_mamba(x))
...
class CNNMobileMambaAttention(nn.Module):
    """CNN + single-direction MobileMamba classifier for inputs of shape (B, 1000, 10)."""
...
def build_cnn_mobilemamba_attention_model(
    input_shape=(1000, 10),
    num_classes=8,
    num_mamba_layers=2,
    mamba_d_state=16,
    ...
    return CNNMobileMambaAttention(
