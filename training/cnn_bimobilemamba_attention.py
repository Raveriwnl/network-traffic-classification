...
class MobileMambaSSMLayer(nn.Module):
    """MobileMamba-style SSM layer adapted to PyTorch."""

    def __init__(self, d_model, d_state=16, d_conv=3, expand=2, input_dim=None):
        super().__init__()
        ...
        self.in_proj = nn.Linear(self.input_dim, self.d_inner * 2, bias=False)
...
class BidirectionalMobileMambaLayer(nn.Module):
    """Bidirectional MobileMamba following the split-output design in old_model.py."""

    def __init__(self, d_model, d_state=16, d_conv=3, expand=2, dropout=0.0):
        super().__init__()
        left_dim = d_model // 2
        right_dim = d_model - left_dim

        self.forward_mamba = MobileMambaSSMLayer(
            d_model=left_dim,
            d_state=d_state,
            d_conv=d_conv,
            expand=expand,
        ...
        self.backward_mamba = MobileMambaSSMLayer(
            d_model=right_dim,
            d_state=d_state,
            d_conv=d_conv,
            expand=expand,
    ...
    def forward(self, x):
        ...
        return self.dropout(torch.cat([forward_states, backward_states], dim=-1))
...
class CNNBiMobileMambaAttention(nn.Module):
    """CNN + bidirectional MobileMamba classifier for inputs of shape (B, 1000, 10)."""
...
def build_cnn_bimobilemamba_attention_model(
    input_shape=(1000, 10),
    num_classes=8,
    num_mamba_layers=2,
    mamba_dropout=0.15,
    ...
    return CNNBiMobileMambaAttention(
