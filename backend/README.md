...
Before startup, ensure PostgreSQL is reachable. The default settings assume PostgreSQL on `127.0.0.1:5432`.

The backend also checks `backend/local.connection.json` first for PostgreSQL connection settings. That file is ignored by git and is intended for local secrets.
